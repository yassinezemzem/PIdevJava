...
public class ServiceProviderService {
    ...
    public void saveServiceProvider(ServiceProvider serviceProvider) {
        String query = "INSERT INTO prestataire (name, last_name, phone, service, latitude, longitude) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public void updateServiceProvider(ServiceProvider serviceProvider) {
        String query = "UPDATE prestataire SET name = ?, last_name = ?, phone = ?, service = ?, latitude = ?, longitude = ? WHERE id = ?";
        try (Connection conn = getConnection();
             PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setString(1, serviceProvider.getName());
            stmt.setString(2, serviceProvider.getLastName());
    ...
    public void deleteServiceProvider(int id) {
        String query = "DELETE FROM prestataire WHERE id = ?";
        try (Connection conn = getConnection();
             PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setInt(1, id);
            stmt.executeUpdate();
